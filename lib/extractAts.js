...
export async function extractGreenhouse(url) {
  ...
  const apiId = data.id != null ? String(data.id) : String(jobId);
...
export async function extractWorkday(url) {
  ...
  const rawId =
    info.jobPostingId ?? info.jobRequisitionId ?? info.id ?? "";
  const wid = rawId !== "" && rawId != null ? String(rawId) : "";
